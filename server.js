...
function sessionUsername(req){const token=readCookie(req,COOKIE_NAME);return token?sessions.get(token):null;}
function isAuthenticated(req){return Boolean(sessionUsername(req));}
function setSession(res,username){const token=randomToken();sessions.set(token,username);res.setHeader('Set-Cookie',`${COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; Secure=${ORIGIN.startsWith('https://')}; SameSite=Lax; Path=/; Max-Age=28800`);}
...
function issueApiKey(username){const key=`ds_${randomToken()}${randomToken()}`;apiKeys.set(hashKey(key),{username,createdAt:new Date().toISOString()});return key;}
app.get('/',(req,res)=>res.redirect(isAuthenticated(req)?'/index.html':'/auth.html'));
app.get('/index.html',(req,res,next)=>isAuthenticated(req)?next():res.redirect('/auth.html'));
app.get('/api/auth/status',(req,res)=>res.json({configured:Boolean(RP_ID&&ORIGIN),enrolled:credentials.size>0,authenticated:isAuthenticated(req),username:sessionUsername(req)||null}));
...
app.post('/api/key/issue',requireSession,(req,res)=>{const key=issueApiKey(sessionUsername(req));res.status(201).json({apiKey:key,warning:'Save this key now. It cannot be recovered from the server.'});});
app.post('/api/auth/logout',(req,res)=>{const token=readCookie(req,COOKIE_NAME);if(token)sessions.delete(token);res.setHeader('Set-Cookie',`${COOKIE_NAME}=; HttpOnly; Secure=${ORIGIN.startsWith('https://')}; SameSite=Lax; Path=/; Max-Age=0`);res.json({ok:true});});
