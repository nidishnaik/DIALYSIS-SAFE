...
// Demo persistence. Replace with a database for production; never put credential
// private keys or API secrets in browser code, HTML, or source control.
...
function setSession(res) {
  ...
  sessions.add(token);
  res.setHeader('Set-Cookie', `${COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=28800`);
}
function requireSession(req, res, next) {
  if (!sessions.has(readCookie(req, COOKIE_NAME))) return res.status(401).json({ error: 'Passkey authentication required' });
  next();
}

app.get('/api/auth/status', (req, res) => res.json({ configured: Boolean(RP_ID && ORIGIN), enrolled: credentials.size > 0, authenticated: sessions.has(readCookie(req, COOKIE_NAME)) }));
...
app.post('/api/auth/register/verify', async (req, res) => {
  ...
  try {
    const verification = await verifyRegistrationResponse({ response: req.body.response, expectedChallenge, expectedOrigin: ORIGIN, expectedRPID: RP_ID, requireUserVerification: true });
    ...
    credentials.set(info.credential.id, { id: info.credential.id, publicKey: info.credential.publicKey, counter: info.credential.counter, username });
...
app.post('/api/auth/login/options', async (_req, res) => {
  const options = await generateAuthenticationOptions({ rpID: RP_ID, userVerification: 'required', allowCredentials: [...credentials.values()].map(c => ({ id: c.id, type: 'public-key' })) });
...
app.post('/api/auth/login/verify', async (req, res) => {
  ...
  try {
    const verification = await verifyAuthenticationResponse({ response: req.body.response, expectedChallenge, expectedOrigin: ORIGIN, expectedRPID: RP_ID, credential: { id: credential.id, publicKey: credential.publicKey, counter: credential.counter }, requireUserVerification: true });
    if (!verification.verified) return res.status(401).json({ error: 'Passkey verification failed' });
    credential.counter = verification.authenticationInfo.newCounter;
    challenges.delete('login');
    setSession(res);
    res.json({ verified: true, username: credential.username });
